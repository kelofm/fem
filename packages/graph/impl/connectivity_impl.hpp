                             ...
                             utils::Comparison<TValue> comparison)
{
    ...
    if (numberOfSamples && ansatzSize) {
        ...
        tsl::robin_map<
            Size,                   ///< ansatz function index on one side
            tsl::robin_set<Size>    ///< indices of ansatz functions with identical values on the other side
        > sameSidePairs, opposingSidePairs;

        // Indices of ansatz functions that don't vanish on the given boundary
        tsl::robin_set<std::pair<BoundaryID,Size>> nonzeros;

        // Loop over dimensions and find coincident ansatz functions on boundary pairs
        for (unsigned iDim=0; iDim<Dimension; ++iDim) {
            ...
            do {
                ...
                for (unsigned i=0; i<Dimension; ++i) {
                    if (i != iDim) {
                        samplePoints.back()[i] = samples[argumentState[iState++]];
                    }
                } // for i in range(Dimension)
            } while (maths::CartesianProduct<Dimension-1>::next(numberOfSamples, argumentState.data()));
            ...
            // (consistent with the assumption of all functions vanishing on both boundaries)
            {
                ...
                for (Size i=0; i<ansatzSize; ++i) {
                    all.insert(i);
                }
                for (Size iAnsatz=0; iAnsatz<ansatzSize; ++iAnsatz) {
                    sameSidePairs.insert_or_assign(iAnsatz, all);
                    opposingSidePairs.insert_or_assign(iAnsatz, all);
                }
            }

            // Loop through the sample points and pick another point as reference.
            // We need to handle two cases:
            // 1) if we expect the boundaries of two elements to be pointing in opposite
            //    directions (the BoundaryIDs match), then we need to assume that one of
            //    the boundaries was rotated (not flipped).

            do {
                // Evaluate the ansatz space on the first boundary
                argument[iDim] = Value(-1);
                rAnsatzSpace.evaluate(argument.begin(),
                                    argument.end(),
                                    valueBuffer.data());

                // Evaluate the ansatz space on the second boundary
                argument[iDim] = Value(1);
                rAnsatzSpace.evaluate(argument.begin(),
                                    argument.end(),
...
void AnsatzMap<TValue>::getPairs(BoundaryID first,
                                 BoundaryID second,
